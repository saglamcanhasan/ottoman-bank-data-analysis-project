import dash
from utils.graph.graph import plot, bar, combine, graph
...
create_figure_callback(lambda **kwargs: request("employee-count", **kwargs), lambda df: graph(plot(df, "Year", "Employee Count", "Number of Employees vs. Year", 0)), "total-employees", True, True, True, True, True)

create_agency_dropdown_callback("agency-number")
create_figure_callback(lambda **kwargs: request("agency-count", **kwargs), lambda df: graph(plot(df, "Year", "Agency Count", "Number of Agencies vs. Year", 0)), "agency-number", True, False, False, False, True)

create_agency_dropdown_callback("employee-turnover")
create_figure_callback(lambda **kwargs: request("employee-turnover", **kwargs), lambda df: graph(combine([bar(df, "Year", "Hires", "", 0), bar(df, "Year", "Departures", "", 6)], [plot(df, "Year", "Net Change", "", 4)], "Year", ["Hires/Departures", "Net Change"], "Number of Hires & Departures vs. Year")), "employee-turnover", True, True, True, True, True)